...
public class Slider2d {
    ...
    public Slider2d(Config config) {
        ...
        layerLength = Optional.ofNullable(data).flatMap(l -> l.stream().map(s -> s.getLayer().size()).max(comp))
                .orElse(0);
    ...
    public List<Double> getData(double x, double y) {
        ...
        List<Pair<List<Double>, Double>> gauss = data.stream()
        ...
        gauss.stream().forEach(p -> IntStream.range(0, p.getKey().size()).forEach(i -> sum[i] += p.getValue()));
        double[] result = new double[this.layerLength];
        gauss.stream().forEach(p -> IntStream.range(0, p.getKey().size())
