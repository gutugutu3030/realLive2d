...
public class Layer {
  ...
  public List<Pair<String, Double>> getServoDefaultAngles() {
    Function<Object, String> getFieldName =
        o -> {
          try {
    ...
    return Stream.of(servoY1, servoY2, servoX)
        .map(s -> new Pair<>(getFieldName.apply(s), s.getDefaultAngle()))
  ...
  public void setServoOffset(ServoOffset offset) {
    try {
      Optional.ofNullable(this.getClass().getDeclaredField(offset.servo))
          .map(
              f -> {
                try {
                  f.setAccessible(true);
          ...
          .filter(Servo.class::isInstance)
          .map(Servo.class::cast)
          .ifPresent(s -> s.setDefaultAngle(offset.defautAngle));
    } catch (Exception e) {
