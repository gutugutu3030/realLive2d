...
public class ArtWebSocketServer extends WebSocketServer {
  ...
  public ArtWebSocketServer(Object methodClass, Config config) {
    super(new InetSocketAddress(Optional.ofNullable(config).map(c -> c.websocket).map(c -> c.port).orElse(8080)));
  ...
  public void onMessage(WebSocket conn, String message) {
    ...
    try {
      Optional.ofNullable(mapper.readValue(message, WebSocketJson.class)).ifPresent(this::onMessageExec);
  ...
  private void onMessageExec(WebSocketJson json) {
    ...
    Arrays.stream(methodClass.getClass().getDeclaredMethods()) //
        .filter(m -> Arrays.stream(m.getDeclaredAnnotations()) //
            .filter(WebSocketMethod.class::isInstance).map(WebSocketMethod.class::cast).findAny() //
            .map(WebSocketMethod::addr).map(json.address::equals).orElse(false))
        .findAny().ifPresent(m -> {
          ...
          try {
            ...
            IntStream.range(0, parameterType.length).allMatch(i -> //
  ...
  public void sendOscBundle(OSCMessage... messages) {
    try {
      this.broadcast(new ObjectMapper().writeValueAsString(Arrays.stream(messages).map(message -> {
        Map<String, Object> map = new HashMap<>();
        map.put("address", message.getAddress());
        map.put("args", message.getArguments());
        return map;
  ...
  public void sendOscBundle(List<? extends OSCMessage> messages) {
    try {
      this.broadcast(new ObjectMapper().writeValueAsString(messages.stream().map(message -> {
        Map<String, Object> map = new HashMap<>();
        map.put("address", message.getAddress());
        map.put("args", message.getArguments());
        return map;
  ...
  public void sendOscBundle(OSCBundle bundle) {
    try {
      this.broadcast(new ObjectMapper().writeValueAsString(
          bundle.getPackets().stream().filter(OSCMessage.class::isInstance).map(OSCMessage.class::cast).map(message -> {
            Map<String, Object> map = new HashMap<>();
            map.put("address", message.getAddress());
            map.put("args", message.getArguments());
            return map;
