package io.github.gutugutu3030.pi;
...
/** サーボをi2cで制御するボードPCA9685を制御するクラスです */
public class PCA9685 {
  /** 1つのPCA9685につなげるサーボの数 */
   ...
   * 複数のPCA9685ボードを用意します
   *
   * @param addresses PCA9685のチャンネル
   */
  public PCA9685(int... addresses) {
    Arrays.stream(addresses)
        .mapToObj(
            i -> {
              try {
                PWMDevice device = new PWMDevice(I2CBus.BUS_1, i);
                device.setPWMFreqency(servoFrequency);
                return device;
              } catch (Exception e) {
                return null;
              }
   ...
   * pwmを設定します
   *
   * @param pwms 設定するpwmのリスト
   */
  public void write(List<Integer> pwms) {
    IntStream.range(0, Math.min(pwms.size(), device.size() * maxOneBoardServoNum))
        .forEach(
            i -> {
              try {
                device
                    .get(i / maxOneBoardServoNum)
                    .getChannel(i % maxOneBoardServoNum)
                    .setPWM(0, pwms.get(i));
              } catch (IOException e) {
                log.error("failed setting servo pwm.", e);
              }
            });
  }
}
...
class PWMDevice {
  ...
  /**
   * Sets the PWM frequency to use. This is common across all channels. For controlling RC servos,
   * 50Hz is a good starting point.
   *
  ...
  public class PWMChannel {
     ...
     *
     * @param on when to go from low to high [0, 4095]. 0 means at the very start of the pulse, 4095
     *     at the very end.
     * @param off when to go from high to low [0, 4095]. 0 means at the very start of the pulse,
     *     4095 at the very end.
     * @throws IOException
     */
