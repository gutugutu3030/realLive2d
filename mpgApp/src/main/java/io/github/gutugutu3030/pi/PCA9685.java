...
public class PCA9685 {
     ...
     */
    private final static int maxOneBoardServoNum = 16;
    ...
    public PCA9685(int... addresses) {
        Arrays.stream(addresses).mapToObj(i -> {
            try {
    ...
    public void write(List<Integer> pwms) {
        IntStream.range(0, Math.min(pwms.size(), device.size() * maxOneBoardServoNum)).forEach(i -> {
            try {
                device.get(i / maxOneBoardServoNum).getChannel(i % maxOneBoardServoNum).setPWM(0, pwms.get(i));
            } catch (IOException e) {
